define([
  ...
  var Game = Class.extend({
        ...
        self.player.onStopPathing(function (x, y) {
          ...
          if (!self.player.hasTarget() && self.map.isDoor(x, y)) {
            ...
            if (isTeleport == 0) {
              ...
              isTeleport = 1;
            } else if (isTeleport == 1) isTeleport = 0;
          }
      ...
      if (!this.buildMode) {
        ...
        if (this.player.build && !this.player.destroy) {
          ...
          if (this.getEntityAt(pos.x, pos.y))
            this.removeHousepoint(
              pos.x,
              pos.y,
              this.getEntityAt(pos.x, pos.y).id
            );
        }
      }
    },
        ...
        if (
          ...
          if (this.player.isAdjacentNonDiagonal(entity) === false) {
            ...
            if (!this.player.disableKeyboardNpcTalk) {
              ...
              if (
                !(
                  this.blockNpc.includes(entity.kind) ||
                  this.teleportNpc.includes(entity.kind)
                )
              )
    ...
    say: function (message) {
      ...
      if (chatPartner) this.makeNpcTalk(chatPartner, message);
      if (message) {
        if (message == "a") {
          this.buildMode = true;
          this.player.build = false;
